...
| 组件 | 协议 | 作者 | 在本仓库的位置 |
| --- | --- | --- | --- |
| **MyCard 天梯模块** | 未单独标注许可；使用受原作者声明约束 | Nanahira（78877@qq.com） | `Assets/SibylSystem/MyCard/`（`MyCard.cs`、`MyCardHelper.cs`、`MyCardCredentialStore.cs`） |

原作者在 `MyCardHelper.cs` 文件头声明：本模块用于通过 MyCard API 接入其天梯排位系统，
**在 fork 或以本模块形式再分发工程之前，应先与 MyCard 方面联系**。

本工程在包含并再分发本模块之前已就此与 MyCard 方面取得联系并获同意。
如你计划 fork 本仓库或以任何形式再分发含有本模块的内容，请自行确认并遵守该要求。

本模块访问的接口为 MyCard 官方服务 `https://sapi.moecube.com:444`。
本工程**不包含**任何账号、密码、令牌或密钥；登录凭据由使用者自行输入，并在本地保存。
