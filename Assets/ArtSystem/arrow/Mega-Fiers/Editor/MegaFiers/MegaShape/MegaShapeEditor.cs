		...
		for ( int s = 0; s < shape.splines.Count; s++ )
		{
			for ( int p = 0; p < shape.splines[s].knots.Count; p++ )
			{
				...
				if ( shape.drawKnots )	//&& recalc == false )
				{
					...
					//shape.splines[s].knots[p].p = Handles.FreeMoveHandle(pm, Quaternion.identity, ksize, Vector3.zero, Handles.SphereHandleCap);	//CubeCap);
					Handles.color = nocol;	//shape.VecCol;
					Vector3 newp = Handles.FreeMoveHandle(pm, Quaternion.identity, knotsize * ringratio, Vector3.zero, Handles.CircleHandleCap);	//SphereCap);	//CubeCap);
				...
				if ( shape.drawHandles )	//&& recalc == false )
				{
					...
					//else
					Handles.color = nocol;
					Vector3 newinvec = Handles.FreeMoveHandle(shape.splines[s].knots[p].invec, Quaternion.identity, handlesize * ringratio, Vector3.zero, Handles.CircleHandleCap);	//SphereCap);	//CubeCap);
					...
					//else
					Vector3 newoutvec = Handles.FreeMoveHandle(shape.splines[s].knots[p].outvec, Quaternion.identity, handlesize * ringratio, Vector3.zero, Handles.CircleHandleCap);	//SphereCap);	//CubeCap);
