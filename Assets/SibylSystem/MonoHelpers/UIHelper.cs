...
public static class UIHelper
{
    ...
    /// <summary>
    /// transAtlas 里 YGOPro2 原生图标（ai / wlan / replay / puzzle / deck / setting / exit）
    /// 画的是 #D6D6D6（214），而从 KoishiPro 借来的那批（link / new / file / home / see …）
    /// 是 #FFFFFF（255）。同一个菜单里混用就会「有几项明显更亮、像是另一个出处」。
    /// UISprite.color 是乘法、只能压不能提，所以把偏白的图标统一乘上 214/255 拉回原生档位。
    /// </summary>
    public const float NativeIconTone = 214f / 255f;

    /// <summary>原生图标那一档的白色（描边/色调统一用这个，别再用 Color.white）。</summary>
    public static Color NativeIconColor
    {
        get { return new Color(NativeIconTone, NativeIconTone, NativeIconTone, 1f); }
    }
