...
public class GameTextureManager
{
    ...
    private static byte[] getPicture(PictureResource pic, out bool EightEdition)
    {
        ...
        // 卡图目录按模式走：OCG → picture/card，RD → rd/picture/card（见 GameModeManager.PictureRoot）。
        // ⚠ 这是「RD 模式下看不到卡图」的根因 —— 原来这里把 "picture/card" 写死，
        //   而 RD 卡图（12xxxxxxxx）铺在 rd/picture/card/，查不到就退化成空白卡背。
        string cardDir = GameModeManager.CardPictureDir;
        string path = cardDir + "/" + code + ".png";
        if (!File.Exists(path))
        {
            path = cardDir + "/" + code + ".jpg";
        }
        // 8 版卡图是 OCG 的历史素材集，RD 没有这一套 ——
        // RD 侧不回落，否则会拿 OCG 早期版本的图去顶 RD 卡（码域不重叠，实际也查不到，纯白发一次 IO）。
        if (!File.Exists(path) && !GameModeManager.IsRD)
        {
            EightEdition = true;
        ...
        if (File.Exists(path))
        {
            using (FileStream file = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                ...
                PicTrace(code, path, data.Length, EightEdition);
                return data;
            }
        }
        PicTrace(code, "(未找到)", 0, EightEdition);
        return new byte[0];
    }

    /// <summary>
    /// 卡图解析轨迹：报「这次去哪找的、找到没有、多少字节」。
    ///
    /// 判据用：**RD 模式下必须 root=rd/picture 且 bytes&gt;0**。
    /// 「RD 看不到卡图」这类问题光看磁盘上有没有文件判不出来 ——
    /// 图铺好了但代码里路径写死成 picture/card，表现就是一张都查不到（空白卡背）。
    /// </summary>
    private static void PicTrace(string code, string path, int bytes, bool eight)
    {
        PicTrace("card", code, path, bytes, eight);
    }

    /// <summary>同上，带 kind（card / closeup）—— 验收脚本按 kind 分开判。</summary>
    private static void PicTrace(string kind, string code, string path, int bytes, bool eight)
    {
        if (!QuickTestTrace.Enabled)
        {
            return;
        }
        QuickTestTrace.Log("pic", "kind=" + kind
            + " code=" + code
            + " root=" + GameModeManager.PictureRoot
            + " bytes=" + bytes
            + " eight=" + (eight ? 1 : 0)
            // path 放行尾：文件名里不会再有空格，但保持与其它探针同一口径（含空格的值一律行尾）。
            + " path=" + path);
    }
